...
if rootWidget then
    ...
    autoPartyListWindow.addPlayer.onClick = function(widget)
        ...
        if playerName:len() > 0 and
            not (table.contains(config.autoPartyList, playerName, true) or config.leaderName == playerName) then
    ...
    onTalk(function(name, level, mode, text, channelId, pos)
        ...
        if not config.onMsg or not string.find(text, config.inviteTxt) then
            return true
        end
